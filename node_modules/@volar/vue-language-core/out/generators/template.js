...
function generate(ts, compilerOptions, vueCompilerOptions, sourceTemplate, sourceLang, templateAst, hasScriptSetup, cssScopedClasses = []) {
    ...
    function declareSlots() {
        ...
        for (const [name, slot] of slots) {
            hasSlot = true;
            writeObjectProperty(name, slot.loc, // TODO: SourceMaps.MappingKind.Expand
            {
    ...
    function writeStyleScopedClasses() {
        ...
        for (const { className, offset } of scopedClasses) {
            ...
            writeCodeWithQuotes(className, offset, {
        ...
        for (const tagName in tagNames) {
            ...
            for (const name of names) {
                for (const tagRange of tagRanges) {
                    ...
                    writePropertyAccess(name, tagRange, {
    ...
    function visitNode(node, parentEl) {
        if (node.type === 0 /* CompilerDOM.NodeTypes.ROOT */) {
            ...
            writeInterpolation(content, start, capabilitiesPresets.all, '(', ');\n', node.content.loc);
            writeInterpolationVarsExtraCompletion();
            const lines = content.split('\n');
            appendFormattingCode(content, start, lines.length <= 1 ? formatBrackets.curly : [
            ...
            for (let i = 0; i < node.branches.length; i++) {
                ...
                if (branch.condition?.type === 4 /* CompilerDOM.NodeTypes.SIMPLE_EXPRESSION */) {
                    codeGen.push(` `);
                    writeInterpolation(branch.condition.content, branch.condition.loc.start.offset, capabilitiesPresets.all, '(', ')', branch.condition.loc);
            ...
            if (source.type === 4 /* CompilerDOM.NodeTypes.SIMPLE_EXPRESSION */) {
                writeInterpolation(source.content, source.loc.start.offset, capabilitiesPresets.all, '(', ')', source.loc);
    ...
    function visitElementNode(node, parentEl) {
        ...
        if (vueCompilerOptions.jsxTemplates) {
            ...
            const tagCapabilities = _isIntrinsicElement || _isNamespacedTag ? capabilitiesPresets.all : {
                ...capabilitiesPresets.diagnosticOnly,
            ...
            if (_isIntrinsicElement) {
                for (const offset of tagOffsets) {
                    ...
                    writePropertyAccess(node.tag, offset, {
                        ...capabilitiesPresets.tagReference,
                        ...capabilitiesPresets.tagHover,
                    });
                    codeGen.push(`;\n`);
                ...
                writeCodeWithQuotes(node.tag, tagOffsets[0], capabilitiesPresets.diagnosticOnly);
                codeGen.push(`] = `);
            }
        ...
        }
        {
            ...
            for (const failedExp of _unWriteExps) {
                writeInterpolation(failedExp.loc.source, failedExp.loc.start.offset, capabilitiesPresets.all, '(', ')', failedExp.loc);
    ...
    function writeEvents(node) {
        ...
        for (const prop of node.props) {
            if (prop.type === 7 /* CompilerDOM.NodeTypes.DIRECTIVE */
                ...
                writeCodeWithQuotes(key_2, [prop.arg.loc.start.offset, prop.arg.loc.end.offset], {
                ...
                codeGen.push(`]> = {\n`);
                {
                    writeObjectProperty(prop.arg.loc.source, prop.arg.loc.start.offset, capabilitiesPresets.event, prop.arg.loc);
                    codeGen.push(`: `);
                ...
                writeInterpolation(prop.exp.content, prop.exp.loc.start.offset, capabilitiesPresets.all, '$event => {(', ')}', prop.exp.loc);
        ...
        for (const prop of node.props) {
            if (prop.type === 7 /* CompilerDOM.NodeTypes.DIRECTIVE */
                ...
                if (attrNameText === undefined
                    || vueCompilerOptions.dataAttributes.some(pattern => (0, minimatch_1.default)(attrNameText, pattern))
                ...
                const propName = isStatic
                    ...
                    && !vueCompilerOptions.htmlAttributes.some(pattern => (0, minimatch_1.default)(attrNameText, pattern))
                ...
                if (!prop.arg) {
                    writePropName(attrNameText, isStatic, [prop.loc.start.offset, prop.loc.start.offset + prop.loc.source.indexOf('=')], getCaps(capabilitiesPresets.attr), prop.loc.name_1 ?? (prop.loc.name_1 = {}));
                }
                else if (prop.exp?.constType === 3 /* CompilerDOM.ConstantTypes.CAN_STRINGIFY */) {
                    writePropName(propName, isStatic, [prop.arg.loc.start.offset, prop.arg.loc.start.offset + attrNameText.length], // patch style attr,
                    {
                    ...
                    writePropName(propName, isStatic, [prop.arg.loc.start.offset, prop.arg.loc.end.offset], {
                        ...getCaps(capabilitiesPresets.attr),
                        rename: {
                            normalize: shared_1.camelize,
                            apply: getRenameApply(attrNameText),
                ...
                if (prop.exp && !(prop.exp.constType === 3 /* CompilerDOM.ConstantTypes.CAN_STRINGIFY */)) { // style='z-index: 2' will compile to {'z-index':'2'}
                    writeInterpolation(prop.exp.loc.source, prop.exp.loc.start.offset, getCaps(capabilitiesPresets.all), '(', ')', prop.exp.loc);
                ...
                if (prop.arg && attrNameText !== propName) {
                    writePropStart(isStatic);
                    writePropName(attrNameText, isStatic, [prop.arg.loc.start.offset, prop.arg.loc.end.offset], {
                        ...getCaps(capabilitiesPresets.attr),
                        rename: {
                            normalize: shared_1.camelize,
                            apply: getRenameApply(attrNameText),
                ...
                if (vueCompilerOptions.dataAttributes.some(pattern => (0, minimatch_1.default)(attrNameText, pattern))
                    || (attrNameText === 'style' && ++styleAttrNum >= 2)
                    || (attrNameText === 'class' && ++classAttrNum >= 2)
                    || (attrNameText === 'name' && node.tag === 'slot') // #2308
                ) {
                ...
                writePropName(propName, true, [prop.loc.start.offset, prop.loc.start.offset + prop.name.length], {
                    ...getCaps(capabilitiesPresets.attr),
                    rename: {
                        normalize: shared_1.camelize,
                ...
                if (attrNameText !== propName) {
                    writePropStart(true);
                    writePropName(attrNameText, true, prop.loc.start.offset, {
                        ...getCaps(capabilitiesPresets.attr),
                        rename: {
                            normalize: shared_1.camelize,
                            apply: getRenameApply(prop.name),
                ...
                if (format === 'jsx')
                    ...
                    codeGen.push('...');
                writeInterpolation(prop.exp.content, prop.exp.loc.start.offset, getCaps(capabilitiesPresets.all), '(', ')', prop.exp.loc);
                const fb = getFormatBrackets(formatBrackets.round);
                if (fb) {
    ...
    function writeImportSlots(node, parentEl, slotBlockVars) {
        ...
        for (const prop of node.props) {
            if (prop.type === 7 /* CompilerDOM.NodeTypes.DIRECTIVE */
                ...
                let isStatic = true;
                if (prop.arg?.type === 4 /* CompilerDOM.NodeTypes.SIMPLE_EXPRESSION */ && prop.arg.content !== '') {
                ...
                if (isStatic) {
                    ...
                    codeGen.push(`[`);
                    writeInterpolation(slotName, argRange[0] + 1, capabilitiesPresets.all, '', '', prop.loc.slot_name ?? (prop.loc.slot_name = {}));
                    codeGen.push(`]`);
                    writeInterpolationVarsExtraCompletion();
                }
                codeGen.push([
    ...
    function writeDirectives(node) {
        for (const prop of node.props) {
            if (prop.type === 7 /* CompilerDOM.NodeTypes.DIRECTIVE */
                ...
                codeGen.push(`)`);
                if (prop.exp?.type === 4 /* CompilerDOM.NodeTypes.SIMPLE_EXPRESSION */) {
                    writeInterpolation(prop.exp.content, prop.exp.loc.start.offset, capabilitiesPresets.all, '(', ')', prop.exp.loc);
                    appendFormattingCode(prop.exp.content, prop.exp.loc.start.offset, formatBrackets.round);
                }
                codeGen.push([
                    '',
    ...
    function writeElReferences(node) {
        for (const prop of node.props) {
            if (prop.type === 6 /* CompilerDOM.NodeTypes.ATTRIBUTE */
                ...
                && prop.value) {
                codeGen.push(`// @ts-ignore\n`);
                writeInterpolation(prop.value.content, prop.value.loc.start.offset + 1, capabilitiesPresets.refAttr, '(', ')', prop.value.loc);
                codeGen.push(`;\n`);
                writeInterpolationVarsExtraCompletion();
            }
        }
    }
    function writeClassScoped(node) {
        for (const prop of node.props) {
            if (prop.type === 6 /* CompilerDOM.NodeTypes.ATTRIBUTE */
                && prop.name === 'class'
                && prop.value) {
    ...
    function writeSlots(node, startTagOffset) {
        ...
        for (const prop of node.props) {
            if (prop.type === 7 /* CompilerDOM.NodeTypes.DIRECTIVE */
                ...
                writeInterpolation(prop.exp.content, prop.exp.loc.start.offset, capabilitiesPresets.attrReference, '(', ')', prop.exp.loc);
                codeGen.push(`;\n`);
                writeInterpolationVarsExtraCompletion();
                break;
            }
        ...
        for (const prop of node.props) {
            if (prop.type === 7 /* CompilerDOM.NodeTypes.DIRECTIVE */
                ...
                writeObjectProperty(prop.arg.content, [prop.arg.loc.start.offset, prop.arg.loc.end.offset], {
                    ...capabilitiesPresets.attrReference,
                    rename: {
                        normalize: shared_1.camelize,
                ...
                writeObjectProperty(prop.name, prop.loc.start.offset, {
